...
which saves low-precision CE state to avoid projection recomputation. When
Megatron-Core is installed, the ``megatron-core`` provider uses its fused CE.
The always-available ``megatron-compatible`` provider uses Liger's drop-in
Megatron CE.
...
from optimization.megatron_fused_linear_cross_entropy.nvshmem.megatron_fused_linear_cross_entropy_nvshmem import (
...
def _make_state(
    ...
    elif provider in ("liger-nvshmem", "liger-nvshmem-device"):
        forward = lambda: liger_megatron_fused_linear_cross_entropy(
            hidden,
            weight,
            target,
            bias=bias,
            tp_group=tp_group,
            nvshmem_workspace=nvshmem_workspace,
        )
    else:
        if tp_size > 1:
...
def _check_correctness(
    ...
    hidden, weight, bias, target = _make_masters(
        ...
        tp_size,
        num_tokens=32,
        hidden_size=nvshmem_workspace.hidden_size if nvshmem_workspace is not None else 256,
        vocab_global=1024,
        dtype=dtype,
        with_bias=True,
        device=device,
    ...
    outputs = {}
    checked_providers = ["megatron-compatible", "liger"]
    if nvshmem_provider is not None:
        checked_providers.append(nvshmem_provider)
    ...
    for provider in checked_providers[1:]:
...
def _worker(
    ...
    nvshmem_workspace = None
    nvshmem_provider = None
    if "liger-nvshmem-device" in providers and tp_size > 1:
        ...
        nvshmem_provider = "liger-nvshmem-device"
        nvshmem_workspace = DeviceNvshmemReductionWorkspace(
    ...
    elif "liger-nvshmem" in providers and tp_size > 1:
        from optimization.megatron_fused_linear_cross_entropy.nvshmem.nvshmem_workspace import (
            NativeNvshmemReductionWorkspace,
        )

        nvshmem_provider = "liger-nvshmem"
        nvshmem_workspace = NativeNvshmemReductionWorkspace(
            max_tokens=max(token_counts),
            hidden_size=hidden_size,
            process_group=tp_group,
            device=device,
    ...
    if nvshmem_workspace is not None:
        nvshmem_workspace.close()
...
def main():
    ...
    providers = args.providers or ["megatron-compatible", "liger", "liger-nvshmem-device"]
